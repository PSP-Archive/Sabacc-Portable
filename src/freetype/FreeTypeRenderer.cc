...
#if defined(_DEBUG) || defined(_DEBUGFREETYPERENDERER) || defined(_DEBUGFREETYPEPOSITIONING) || defined(_DEBUGGLYPHCACHE) || defined(_DEBUGFREETYPEALPHA) || defined(_DEBUGENTRY)
...
void FreeTypeRenderer::setColor(int red, int green, int blue, int alpha) {
	...
	static_cast<Uint8>(red), static_cast<Uint8>(green),
...
int FreeTypeRenderer::lineWidth(string text) {
...
int
FreeTypeRenderer::textWidth(string text) {
    int largest_line = 0;
...
int
FreeTypeRenderer::textHeight(string text) {
...
SmartSurface
FreeTypeRenderer::render(string text) {
...
void
FreeTypeRenderer::render(string text, SDL_Surface* destination, int dx, int dy) {

#if defined(_DEBUGENTRY)
    logAppend("render()");
#endif
	...
	for (string::const_iterator char_iter = line.begin(); line.end() != char_iter; ++char_iter) {
		...
		for (int row = 0; str_glyph->bitmap.rows > row; ++row) {
		    ...
		    Uint8* src = static_cast<Uint8*>(str_glyph->bitmap.buffer) 
			+ str_glyph->bitmap.pitch * row;
		    Uint32* dst = static_cast<Uint32*>(destination->pixels) 
			+ (row + y_pos 
			   + (current_state.getAscent() 
			      - ftFloor(glyph->metrics.horiBearingY))) 
			* destination->pitch/bpp + (x_pos + glyph->x_min);

		    for (int col = 0;
			 (str_glyph->bitmap.width > col) && (dst < dst_check);
			 ++col) {

			Uint8 dst_red, dst_green, dst_blue, dst_alpha;

			SDL_GetRGBA(*dst, destination->format,
...
CachedGlyph FreeTypeRenderer::loadGlyph(char glyph_char) {
    ...
    sprintf(debug_string,
	    "Retrieved character glyph index: %d.", temp_cglyph->index);
    logAppend(debug_string);
#endif
	...
	shear.xx = 1 << 16;
	shear.xy = static_cast<int>(current_state.getItalicOffset()
				    * (1 << 16)) / current_state.getHeight();
	shear.yx = 0;
	shear.yy = 1 << 16;
...
CachedGlyph FreeTypeRenderer::loadStrokedGlyph(char glyph_char) {
	...
	throw
	    Exceptions::FreeType::LoadGlyph("Could not open library routine to create stroke when stroke style requested",
					    freetype_error,
					    __FILE__,
					    __LINE__);
    }
...
#if defined(_DEBUGFREETYPERENDERER) || defined(_DEBUGGLYPHCACHE)
    sprintf(debug_string, "Retrieved character glyph index: %d.",
	    temp_cglyph->index);
    logAppend(debug_string);
#endif
	...
	throw
	    Exceptions::FreeType::GeneralError(__FILE__,
					       "Could not create italicized glyph.");
    }	// if(result)
